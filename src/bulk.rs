...
fn bulk_ws_url(hub_base: &str, bulk_id: u32) -> String {
    ...
    format!("{ws}/agent/bulk?id={bulk_id}")
}
...
pub async fn serve(state: AppState, hub_base: String, token: Option<String>, bulk_id: u32, spec: BulkSpec) {
...
async fn run(
    ...
    let url = bulk_ws_url(hub_base, bulk_id);
...
mod tests {
    ...
    #[test]
    fn bulk_ws_url_swaps_scheme_and_appends_id() {
        assert_eq!(bulk_ws_url("http://hub:8840", 7), "ws://hub:8840/agent/bulk?id=7");
        assert_eq!(bulk_ws_url("https://hub.ts.net", 3), "wss://hub.ts.net/agent/bulk?id=3");
        assert_eq!(bulk_ws_url("http://hub:8840/", 1), "ws://hub:8840/agent/bulk?id=1");
    }
}
