...
//!
//! Shared by the agent (for directly-connected clients) and the hub (centralized
//! across the fleet). Pieces:
//!
...
//!     `web-push-native` (pure-Rust RustCrypto + jwt-simple) and POSTs it to each
//!     subscription's endpoint via `ureq` (pure-Rust rustls+ring). Dead endpoints
//!     (404/410) are pruned.
//!
//! The busy→waiting *edge* that triggers a push lives elsewhere: in the agent's
//! `finish_watcher` (for direct clients) and, under a hub, in the uplink's
//! session poll (which emits `WaitingEdge`, and the hub calls `notify`).
